(function () {
    'use strict';
    const userNameInput = document.getElementById('user-name');//入力された名前を取得
    const assessmentButton = document.getElementById('assessment');//「診断する」クリックを取得
     ...
     */
    function removeAllChildren(element) {//removeAllChildren機能の定義
        while (element.firstChild) { // 子どもの要素があるかぎり削除
            element.removeChild(element.firstChild);
        }
    }
    userNameInput.onkeydown = (event) => {
        if (event.keyCode === 13) {// TODO ボタンのonclick() 処理を呼び出す
     ...
     * @return {string} 診断結果
     */
    function assessment(userName) {//userNameを診断するという機能
